...
async def run_autonomous_agent(
    ...
    # Read actual task_type from implementation_plan.json for correct UI display
    _bridge_task_type = "impl"
    try:
        _bridge_plan = load_implementation_plan(spec_dir)
        if _bridge_plan:
            _bridge_task_type = _bridge_plan.get("taskType", _bridge_plan.get("task_type", "impl"))
    except Exception:
        pass
    bridge = DaemonStatusBridge(bridge_project_dir, spec_dir.name, spec_dir, task_type=_bridge_task_type)
    bridge.start()
    ...
    *,
    ...
    while True:
        ...
        if is_planning_phase and status != "error":
            # Check if this is a design/architecture task that creates child specs
            # instead of phases/subtasks. These tasks should NOT go through plan
            # validation because they never produce a standard implementation plan.
            _plan_file = spec_dir / "implementation_plan.json"
            _task_type = ""
            try:
                with open(_plan_file, encoding="utf-8") as _pf:
                    _current_plan = json.load(_pf)
                _task_type = _current_plan.get("taskType") or _current_plan.get("task_type", "")
            except (OSError, json.JSONDecodeError):
                _current_plan = {}

            if _task_type in ("design", "architecture"):
                # Design tasks create child specs via create_batch_child_specs tool.
                # Check multiple sources for evidence of child spec creation:
                #  1. childSpecs field in plan JSON (set by MCP tool)
                #  2. status == "complete" in plan JSON (set by MCP tool)
                #  3. Actual child spec folders on disk (most reliable fallback)
                child_specs = _current_plan.get("childSpecs", [])
                plan_status = _current_plan.get("status", "")

                # Compute original project dir (needed for both scan and write)
                _orig_project = None
                _pdir_str = str(project_dir.resolve()).replace("\\", "/")
                for _wt_marker in [
                    "/.auto-claude/worktrees/tasks/",
                    "/.auto-claude/github/pr/worktrees/",
                    "/.worktrees/",
                ]:
                    if _wt_marker in _pdir_str:
                        _orig_project = Path(_pdir_str.split(_wt_marker)[0])
                        break

                # Fallback: scan disk for child spec folders with parentTask
                # The MCP tool may write child specs to the original project dir
                # while the plan JSON is in a worktree — so plan JSON may not be updated.
                if not child_specs and plan_status != "complete":
                    spec_id = spec_dir.name
                    # Check both worktree and original project specs dirs
                    specs_dirs_to_check = {spec_dir.parent}
                    if _orig_project and _orig_project != project_dir:
                        specs_dirs_to_check.add(
                            _orig_project / ".auto-claude" / "specs"
                        )
                    for _specs_parent in specs_dirs_to_check:
                        if not _specs_parent.is_dir():
                            continue
                        try:
                            for _sibling in _specs_parent.iterdir():
                                if not _sibling.is_dir() or _sibling.name == spec_id:
                                    continue
                                _sib_plan = _sibling / "implementation_plan.json"
                                if _sib_plan.exists():
                                    try:
                                        with open(_sib_plan, encoding="utf-8") as _sf:
                                            _sp = json.load(_sf)
                                        _parent = _sp.get("parentTask") or _sp.get("parent_task")
                                        if _parent == spec_id:
                                            child_specs.append(_sibling.name)
                                    except (OSError, json.JSONDecodeError):
                                        continue
                        except OSError:
                            continue

                if child_specs or plan_status == "complete":
                    plan_validated = True
                    planning_retry_context = None
                    print_status(
                        f"Design task completed — {len(child_specs)} child specs created",
                        "success",
                    )
                    # Update parent plan to "complete" (may have failed in MCP tool)
                    _current_plan["status"] = "complete"
                    _current_plan["planStatus"] = "complete"
                    _current_plan["executionPhase"] = "complete"
                    _current_plan["childSpecs"] = child_specs
                    # Write to worktree spec dir
                    try:
                        with open(_plan_file, "w", encoding="utf-8") as _wf:
                            json.dump(_current_plan, _wf, indent=2)
                    except OSError:
                        pass
                    # Also write to original spec dir (critical for daemon)
                    if _orig_project and _orig_project != project_dir:
                        _orig_plan = (
                            _orig_project / ".auto-claude" / "specs"
                            / spec_dir.name / "implementation_plan.json"
                        )
                        try:
                            with open(_orig_plan, "w", encoding="utf-8") as _wf:
                                json.dump(_current_plan, _wf, indent=2)
                            print_status(
                                "Parent plan updated in original project dir", "success"
                            )
                        except OSError:
                            pass
                    # Force status to "complete" so the loop breaks below
                    status = "complete"
                else:
                    # Agent didn't create child specs — retry
                    planning_validation_failures += 1
                    if planning_validation_failures >= max_planning_validation_retries:
                        print_status(
                            "Design agent failed to create child specs after multiple attempts",
                            "error",
                        )
                    ...
                    print_status(
                        "Design agent didn't create child specs — retrying planner",
                        "warning",
                    )
                    first_run = True
                    status = "continue"
            else:
                # Normal tasks: validate implementation plan schema
