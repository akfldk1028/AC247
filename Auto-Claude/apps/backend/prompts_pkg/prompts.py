...
def is_first_run(spec_dir: Path) -> bool:
    ...
    try:
        ...
        # If childSpecs exist in plan JSON, the design agent already did its job
        child_specs = plan.get("childSpecs", [])
        if child_specs:
            return False

        # Fallback: check if actual child spec folders exist on disk
        # (in case the plan JSON wasn't updated due to write failure)
        if task_type in ("design", "architecture"):
            spec_id = spec_dir.name
            specs_parent = spec_dir.parent  # .auto-claude/specs/
            try:
                for sibling in specs_parent.iterdir():
                    if not sibling.is_dir() or sibling.name == spec_id:
                        continue
                    sibling_plan = sibling / "implementation_plan.json"
                    if sibling_plan.exists():
                        try:
                            with open(sibling_plan, encoding="utf-8") as sf:
                                sp = json.load(sf)
                            parent = sp.get("parentTask") or sp.get("parent_task")
                            if parent == spec_id:
                                return False  # Child spec found on disk
...
def get_custom_agent_prompt(agent_type: str, spec_dir: Path, project_dir: Path) -> str | None:
    """
    Load prompt for a custom agent with spec context injected.

    Custom agents are defined in custom_agents/config.json and their prompts
    are stored in custom_agents/prompts/*.md.

    Args:
        agent_type: The custom agent type identifier (e.g., 's3_backend_auth')
        spec_dir: Directory containing the spec files
        project_dir: Root directory of the project

    Returns:
        The custom agent prompt with spec context, or None if not a custom agent
    """
    # Import here to avoid circular imports
    from agents.tools_pkg.models import get_custom_agent_prompt as get_raw_prompt

    raw_prompt = get_raw_prompt(agent_type)
...
def is_custom_agent(agent_type: str) -> bool:
    ...
    Returns:
        True if it's a custom agent, False otherwise
    """
    from agents.tools_pkg.models import AGENT_CONFIGS
    if agent_type not in AGENT_CONFIGS:
        return False
    return AGENT_CONFIGS[agent_type].get("_custom", False)
