...
class ProjectAnalyzer:
    ...
    def analyze(self, force: bool = False) -> SecurityProfile:
        ...
        # Run detection
        self._detect_stack()
        self._detect_language_from_spec()  # Also check spec files for language hints
    ...
    def _detect_language_from_spec(self) -> None:
        ...
        This is important for new/empty projects where the coder agent
        is about to create source files. Without this, commands like
        'python' would be blocked even for Python projects.
        """
        import re

        # Language patterns to detect (case-insensitive)
        # Note: Patterns are ordered by specificity to reduce false positives
        language_patterns = {
            "python": [
                r"\bpython\b", r"\.py\b", r"\bpip\b", r"\bpytest\b",
                r"\bdjango\b", r"\bflask\b", r"\bfastapi\b", r"\bpyproject\.toml\b",
            ],
            "javascript": [
                r"\bjavascript\b", r"\.js\b", r"\bnode\.js\b", r"\bnpm\b",
                r"\breact\b", r"\bvue\b", r"\bangular\b", r"\bexpress\b",
            ],
            "typescript": [
                r"\btypescript\b", r"\.ts\b", r"\.tsx\b", r"\btsconfig\b",
            ],
            "rust": [
                r"\brust\b", r"\bcargo\b", r"\.rs\b", r"\brustc\b",
            ],
            "go": [
                # More specific patterns to avoid "go" as English word
                r"\bgolang\b", r"\.go\b", r"\bgo\.mod\b", r"\bgo\s+build\b",
                r"\bgo\s+run\b", r"\bgo\s+test\b", r"\bgo\s+get\b",
            ],
            "java": [
                r"\bjava\b", r"\.java\b", r"\bmaven\b", r"\bgradle\b", r"\bspring\b",
            ],
            "ruby": [
                r"\bruby\b", r"\.rb\b", r"\brails\b", r"\bbundler\b", r"\bgem\b",
            ],
            "dart": [
                r"\bdart\b", r"\bflutter\b", r"\.dart\b", r"\bpubspec\b",
            ],
            "csharp": [
                r"\bc#\b", r"\bcsharp\b", r"\.cs\b", r"\bdotnet\b", r"\.net\s",
            ],
        }

        # Collect files to check (deduplicated)
        files_to_check: set[Path] = set()

        # Glob patterns for project-wide specs
        for glob_pattern in [
            ".auto-claude/specs/*/spec.md",
            ".auto-claude/specs/*/requirements.json",
        ]:
            files_to_check.update(self.project_dir.glob(glob_pattern))

        # Direct spec_dir files (worktree case)
        if self.spec_dir:
            for filename in ["spec.md", "requirements.json"]:
        ...
        for filepath in files_to_check:
            try:
                content_to_check += filepath.read_text(encoding="utf-8") + "\n"
        ...
        detected_languages = self.profile.detected_stack.languages
        ...
        for lang, patterns in language_patterns.items():
            ...
            for pattern in patterns:
                if re.search(pattern, content_lower):
                    ...
                    print(f"  Detected {lang} from spec file content")
                    break
